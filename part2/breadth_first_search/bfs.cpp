    ...
    int *distances)
{
    if(g->num_edges / g->num_nodes < 5) {
        int local_cnt;
        #pragma omp for private(local_cnt)
        for (int i = 0; i < frontier->count; i++)
        {
            ...
            #pragma omp critical
            {
                memcpy(new_frontier->vertices + new_frontier->count, local_frontier, sizeof(int) * local_cnt);
                new_frontier->count += local_cnt;
            }
    ...
    {
        int local_cnt;
        #pragma omp parallel private(local_cnt)
        {
        local_cnt = 0;
        Vertex* local_frontier = new Vertex [g->num_nodes];
